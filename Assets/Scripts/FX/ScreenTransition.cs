...
public class ScreenTransition : MonoBehaviour
{
    ...
    public static void ShowBanner(string title, string subtitle, float holdSeconds, bool alarm = false, Color? titleColor = null)
    {
        if (Instance == null) return;
        if (Instance._bannerRoutine != null) Instance.StopCoroutine(Instance._bannerRoutine);
        Instance._bannerRoutine = Instance.StartCoroutine(Instance.Banner(title, subtitle, holdSeconds, alarm, titleColor));
    }
    ...
    IEnumerator Banner(string title, string subtitle, float holdSeconds, bool alarm, Color? titleColor)
    {
        _title.text = title;
        // El color de acento del ala del castillo (RoomIdentity) manda sobre el crema;
        // la alarma de captura siempre va en carmesí.
        _title.color = alarm ? Crimson : (titleColor ?? Cream);
